...
import com.waytous.cloud.gateway.config.KafkaProperties;
...
class CloudGatewayKafkaResourceIT {
    ...
    void setup() {
        ...
        CloudGatewayKafkaResource kafkaResource = new CloudGatewayKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/cloud-gateway-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/cloud-gateway-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
